...
def create_database():
    ...
    db_cursor.execute("""CREATE TABLE IF NOT EXISTS
        ...
        name VARCHAR(255), role VARCHAR(255), salary int)""")
...
def add_employee():
    ...
    db_cursor.execute("""INSERT INTO employees (name, role, salary)
                      VALUES (%s, %s, %s)""", (name, role, salary))
    database.commit()
...
def update_employee():
    ...
    role = input("Enter employee role: ")
    salary = input("Enter employee salary: ")
    db_cursor.execute("UPDATE employees SET name = %s, role = %s, salary = %s",
                      (name, role, salary, id))
    database.commit()
